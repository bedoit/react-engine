...
exports.boot = function boot(options, callback) {
  ...
  if (useRouter) {
    ...
    match({ history: history, routes: options.routes, location: location }, function(error, redirectLocation, renderProps) {
      ...
      // HERE PROPS FROM window.__REACT_ENGINE__ instead props from rout
      // like at Server Side
      ...
      var appComponent = options.appComponent;
      var appElement = React.createElement(
        appComponent, {initData: renderProps});
      ReactDOM.render(appElement, mountNode);
    });
