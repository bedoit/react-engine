...
exports.boot = function boot(options, callback) {
  ...
  try {
    ...
    if (!Provider && options.configureStore) {
      throw err;
    }
  }
  ...
  if (useRouter) {
    ...
    match({ history: history, routes: options.routes, location: location }, function() {

      // for any component created by react-router,
      // merge model data with the routerProps
      // NOTE: This may be imposing too large of an opinion?

      var store = options.configureStore(exports.data().initialStoreData);

      var routerComponent = React.createElement(RouterComponent, {
        createElement: function(Component, routerProps) {
          return React.createElement(Component, merge({store: store}, props, routerProps));
        },
      ...
      // This is must have for integrate with redux
      var _Provider = React.createElement(
        Provider,
        {store: store},
        routerComponent
      );

      ReactDOM.render(_Provider, mountNode);
    });
